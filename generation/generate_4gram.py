...
def perplexity(word_probs, sentence):
    ...
    for i in range(len(sentence) - 3):
        ...
        word_4 = sentence[i + 3]
        try:
            word_prob = word_probs[word_1][word_2][word_3][word_4] # probability of current word triplet
        except:
...
def generate_sentence(word_probs, max_words):
    ...
    p2_word = "<s>"
    p1_word = "<s>"
    sentence = p3_word + " " + p2_word + " " + p1_word + " "

    for i in range(max_words):
        word_prob = word_probs[p3_word][p2_word][p1_word]
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(directory):
        ...
        with open(os.path.join(directory, filename), "r") as ifile:
            ...
            for line in ifile:
                ...
                for word in sentence[1:]:
                    ...
                    if p3_word not in word_probs:
                        word_probs[p3_word] = dict()
                    if p2_word not in word_probs[p3_word]:
                        word_probs[p3_word][p2_word] = dict()
                    if p1_word not in word_probs[p3_word][p2_word]:
                        word_probs[p3_word][p2_word][p1_word] = dict()

                    # if we have not seen this word combination, initialize count to 1
                    if word not in word_probs[p3_word][p2_word][p1_word]:
                        word_probs[p3_word][p2_word][p1_word][word] = 1
                    else:
                        word_probs[p3_word][p2_word][p1_word][word] += 1 # increment the count of this 4-word run
        ...
        # normalize counts
        for p3_word in word_probs:
            for p2_word in word_probs[p3_word]:
                for p1_word in word_probs[p3_word][p2_word]:
                    tot = sum(word_probs[p3_word][p2_word][p1_word].values())
                    for word in word_probs[p3_word][p2_word][p1_word]:
                        word_probs[p3_word][p2_word][p1_word][word] = word_probs[p3_word][p2_word][p1_word][word] / tot

        # generate sentences
        num_sentences = 100
        max_length = 50
        sent_file = open("%s/../data/generated_data/4gram/%s" % (os.getcwd(), filename), "w")
        perp_file = open("%s/../data/generated_data/4gram/perp_%s" % (os.getcwd(), filename), "w")
